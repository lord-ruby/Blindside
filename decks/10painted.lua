    ...
    apply = function(self)
        ...
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.8, func = function()
            ...
            for i = 1, #keys_to_beta do
                keys_to_beta[i]:set_ability("m_bld_flip")
            end
            for i = 1, #keys_to_flip do
                keys_to_flip[i]:set_ability("m_bld_sharp")
            end
            for i = 1, #keys_to_hook do
                keys_to_hook[i]:set_ability("m_bld_fossil")
            end
            for i = 1, #keys_to_pot do
                keys_to_pot[i]:set_ability("m_bld_pot")
            end
            for i = 1, #keys_to_snow do
                keys_to_snow[i]:set_ability("m_bld_ball")
            end
