SMODS.Consumable {
    ...
    use = function(self, card, area)
        ...
        for i=1, #G.hand.highlighted do
            G.E_MANAGER:add_event(Event({trigger = 'after',delay = 0.2,func = function() G.hand.highlighted[i]:set_ability(enhancements[i]);upgrade_blind(G.hand.highlighted[i]);return true end }))
        end 
        for i=1, #G.hand.highlighted do
