...
bool within(vec2 center, vec2 sec, float dist) {
    return distance(center, sec) < dist;
}

#define PI 3.14159

float easeinout(float x, float r) {
    return -(cos(PI*(x/r))-1) / 2;
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    if (shiny.y > shiny.y*2) {
        uv = shiny;
    }

    vec4 lighten_add = vec4(0.1);
    // lighten_add = vec4(0.2,-0.2,-0.2,1);

    float modified_y = shiny.y;
    float x = pow(uv.x+uv.y,0.5)-modified_y;
    x *= PI;
    float res = (sin(x) * (sin(10*x)+1)) - 0.4;

    if (res > 0) {
        float impact_factor = tex.a;
        tex += (ceil(res*res) * lighten_add) * impact_factor;
    }
